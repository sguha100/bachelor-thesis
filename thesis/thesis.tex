...
\cite{bengtsson2004timed} implement many common functions on DBM and
...
functions $f, G, F, g$ exist such that
the question `Are states $s_P$ and $s_Q$ in timed automata $P$ and $Q$
related under $R$? ` can have three possible answers:
\begin{enumerate}
\item yes
\item no
\item if and only if each state from the sequence $f(s_P)$ is
  related under R to at least one element from the corresponding
  set from the sequence $G(s_Q)$ and at least one element from each
  set in the sequence $F(s_P)$ is related to the corresponding state
  from the sequence $g(s_Q)$.
\end{enumerate}
  ...
  \Begin{
      \eIf{\lookup{$yes\_ table$, $s_{P}$, $s_{Q}$}}{
        ...
        \eIf{\lookup{$yes\_ table$, $s_{P}$, $s_{Q}$}}{
          ...
          Set $v_P$\;
          \ForEach{$(s_P', L_Q')$ in $(f(s_P), G(s_Q))$}{
            Reset $v_P$\;
            \ForEach{$s_Q'$ in $L_Q'$}{
          ...
          Set $v_Q$\;
          \ForEach{$(s_Q', L_P')$ in $(F(s_P), g(s_Q))$}{
            Reset $v_Q$\;
            \ForEach{$s_P'$ in $L_P'$}{
              \If{\CheckStatesRelation{$P$, $Q$, $s_{P}'$, $s_{Q}'$,
                  $yes\_ table$, $no\_ table$}}{
